...
<body>
    <div class="container">
        ...
        <div class="main-content">
            <div class="header">
                <h1>👤 <%= request.getParameter("id") != null ? "Editar" : "Crear" %> Usuario</h1>
            ...
            <form action="UsuarioSistemaControlador" method="post" id="formUsuario">
                <input type="hidden" name="accion" value="<%= request.getParameter("id") != null ? "actualizarUsuario" : "crearUsuario" %>">
                    ...
                    <div class="form-grid">
                        <div class="form-group">
                            ...
                            <input type="text" id="usuario" name="usuario" class="form-control" 
                                   ...
                                   required maxlength="50" style="text-transform: lowercase;">
                        </div>
                        
                        <div class="form-group">
                            <label for="password">Contraseña: <%= request.getParameter("id") != null ? "" : "*" %></label>
                            <input type="password" id="password" name="password" class="form-control" 
                                   placeholder="<%= request.getParameter("id") != null ? "Dejar vacío para mantener actual" : "Mínimo 8 caracteres" %>" 
                                   <%= request.getParameter("id") == null ? "required" : "" %> 
                                   minlength="8" onkeyup="evaluarPassword()">
                        ...
                        <div class="form-group">
                            <label for="confirmPassword">Confirmar Contraseña: <%= request.getParameter("id") != null ? "" : "*" %></label>
                            <input type="password" id="confirmPassword" name="confirmPassword" class="form-control" 
                                   placeholder="<%= request.getParameter("id") != null ? "Confirmar nueva contraseña" : "Repetir contraseña" %>" 
                                   <%= request.getParameter("id") == null ? "required" : "" %> 
                                   minlength="8">
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <select id="estado" name="estado" class="form-control">
                                ...
                                <option value="SUSPENDIDO" <%= "SUSPENDIDO".equals(request.getParameter("estado")) ? "selected" : "" %>>⏸️ Suspendido</option>
                            </select>
                        </div>
                    </div>
                </div>
                ...
                <div class="form-panel">
                    ...
                    <div class="permissions-panel" id="permissionsPanel" style="display: none;">
                        <h4 style="margin-bottom: 20px;">🔑 Permisos Específicos</h4>
                ...
                <div class="actions-section">
                    ...
                    <div style="display: flex; gap: 15px;">
                        ...
                        <button type="submit" class="btn btn-success" id="btnGuardar">
                            💾 <%= request.getParameter("id") != null ? "Actualizar" : "Crear" %> Usuario
                        </button>
                    </div>
                </div>
            </form>
    ...
    <script>
        ...
        const permisosPorRol = {
            'ADMIN': [
                { grupo: 'Sistema', permisos: ['Gestión de Usuarios', 'Configuración Global', 'Respaldos', 'Logs del Sistema'] },
                { grupo: 'Datos', permisos: ['Clientes', 'Mascotas', 'Servicios', 'Reportes Completos'] },
                { grupo: 'Operaciones', permisos: ['Facturación', 'Pagos', 'Promociones', 'Notificaciones'] },
                { grupo: 'Personal', permisos: ['Gestión de Groomers', 'Horarios', 'Evaluaciones'] }
            ],
            'VETERINARIO': [
                { grupo: 'Consultas', permisos: ['Crear Consulta', 'Historial Médico', 'Diagnósticos', 'Recetas'] },
                { grupo: 'Datos', permisos: ['Ver Clientes', 'Ver Mascotas', 'Actualizar Historiales'] },
                { grupo: 'Reportes', permisos: ['Reportes Médicos', 'Estadísticas de Salud'] }
            ],
            'GROOMER': [
                { grupo: 'Servicios', permisos: ['Servicios de Grooming', 'Registrar Procedimientos', 'Fotos Antes/Después'] },
                { grupo: 'Citas', permisos: ['Ver Citas Asignadas', 'Marcar como Completado', 'Notas de Servicio'] },
                { grupo: 'Inventario', permisos: ['Ver Productos', 'Solicitar Materiales'] }
            ],
            'RECEPCIONISTA': [
                { grupo: 'Atención', permisos: ['Registrar Clientes', 'Agendar Citas', 'Cola de Atención'] },
                { grupo: 'Pagos', permisos: ['Procesar Pagos', 'Generar Facturas', 'Consultar Precios'] },
                { grupo: 'Comunicación', permisos: ['Enviar Notificaciones', 'Confirmar Citas', 'Seguimientos'] }
            ]
        };
        ...
        document.querySelectorAll('.role-option').forEach(option => {
            option.addEventListener('click', function() {
                ...
                // Mostrar permisos correspondientes
                mostrarPermisos(rol);
            });
        });
        ...
        function mostrarPermisos(rol) {
            ...
            if (permisosPorRol[rol]) {
                ...
                grid.innerHTML = permisosPorRol[rol].map(grupo => `
                    <div class="permission-group">
                        <div class="permission-title">${grupo.grupo}</div>
                        ${grupo.permisos.map(permiso => `
                            <div class="permission-item">
                                <input type="checkbox" class="permission-checkbox" 
                                       name="permisos" value="${permiso}" 
                                       id="perm_${permiso.replace(/\s/g, '_')}" checked>
                                <label for="perm_${permiso.replace(/\s/g, '_')}">${permiso}</label>
                            </div>
                        `).join('')}
                    </div>
                `).join('');
        ...
        document.getElementById('nombre').addEventListener('blur', function() {
            ...
            if (!usuarioInput.value.trim()) {
                const nombres = this.value.trim().toLowerCase().split(' ');
                if (nombres.length >= 2) {
                    const usuario = nombres[0].charAt(0) + nombres[nombres.length - 1];
                    usuarioInput.value = usuario.replace(/[^a-z0-9]/g, '');
                }
            }
        });
        ...
        document.getElementById('formUsuario').addEventListener('submit', function(e) {
            ...
            const isNew = !document.querySelector('input[name="id"]');
        ...
        document.addEventListener('DOMContentLoaded', function() {
            const rolActual = '<%= request.getParameter("rol") != null ? request.getParameter("rol") : "" %>';
            if (rolActual) {
                document.querySelector(`[data-role="${rolActual}"]`).click();
            }
        });
    </script>
</body>
