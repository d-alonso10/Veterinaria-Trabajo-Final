...
<html lang="es">
    ...
    <body>
        ...
        <script>
        // --- FUNCIONES GLOBALES ---
        // Deben estar aquí (fuera del listener) para que 'onclick' las encuentre.
    
        /**
         * Filtra la tabla de mascotas por especie (se llama desde los botones onclick).
         */
        ...
        // --- LISTENER PRINCIPAL (SOLO UNO) ---
        // Todo el código de inicialización va aquí.
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // 2. Lógica del Formulario de Búsqueda (CORREGIDA)
            ...
            if (searchForm) { 
                searchForm.addEventListener('submit', function(e) {
                    ...
                    // 'e.submitter' es el botón que envió el form (si es compatible)
                    // Esta lógica previene enviar un POST vacío si se presiona "Buscar"
                    if (!termino && (!e.submitter || e.submitter.type === 'submit')) {
                        e.preventDefault(); // Detenemos el envío
                        // Redirigimos al *Controlador* para que cargue todas las mascotas
                        window.location.href = '<%= request.getContextPath() %>/MascotaControlador?accion=listarTodas';
                    }
                    // Si hay término, el formulario se envía (POST a accion=buscar)
                    // Si se presionó "Ver Todas" (que es un <a>), esta lógica no se ejecuta.
                });
            }

            if (searchInput) {
                // 3. Búsqueda rápida con Enter
            ...
            // 6. Creación de Filtros (CORREGIDO)
            // Esto ahora está dentro del listener principal, no en uno anidado.
            <% if (mascotas != null && !mascotas.isEmpty()) { %>
                ...
                if (tableContainer) { 
                    ...
                    filterDiv.innerHTML = `
                        ...
                        <button onclick="filtrarPorEspecie('todas')" class="btn btn-small btn-primary">Todas</button>
                        <button onclick="filtrarPorEspecie('perro')" class="btn btn-small btn-secondary">🐕 Perros</button>
                        <button onclick="filtrarPorEspecie('gato')" class="btn btn-small btn-secondary">🐈 Gatos</button>
                        <button onclick="filtrarPorEspecie('otro')" class="btn btn-small btn-secondary">🐾 Otros</button>
                    `;
